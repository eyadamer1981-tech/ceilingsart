...
class Service {
    ...
    private $images_table = "service_images";
    ...
    public function addDetailImage($image_path, $sort_order = 0) {
        $query = "INSERT INTO " . $this->images_table . " SET service_id=:service_id, image_path=:image_path, sort_order=:sort_order";
        $stmt = $this->conn->prepare($query);

        $stmt->bindParam(":service_id", $this->id);
    ...
    public function getDetailImages() {
        $query = "SELECT id, image_path, sort_order FROM " . $this->images_table . " WHERE service_id = ? ORDER BY sort_order";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(1, $this->id);
        $stmt->execute();
    ...
    public function deleteDetailImages() {
        $query = "DELETE FROM " . $this->images_table . " WHERE service_id = ?";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(1, $this->id);
